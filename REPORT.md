...
| 질문 원문 | 확인에 사용한 그래프·파일 |
|---|---|
| “2023~2025년 두 팀의 시즌 성적은 어떻게 변화했는가?” | `01_yearly_win_rate.png`, `season_summary.csv` |
| “두 팀은 시즌 중 어느 달에 상대적으로 강하거나 약했는가?” | `02_monthly_win_rate.png`, `monthly_summary.csv` |
| “주요 선수 구자욱과 김도영의 기록은 시즌별로 어떻게 변화했는가?” | `03_player_rate_trend.png`, `04_player_count_trend.png`, `player_stats.csv` |
...
분석에 직접 사용한 파일은 `data/processed/game_results.csv`이며, 864행이다. 주요 컬럼은 `date`(경기일), `team`(분석 팀), `opponent`(상대), `runs_for`·`runs_against`(득점), `result`(W/L/D), `month`(월), `game_id`(경기 식별자)이다.
...
| 점검 항목 | 결과 | 처리 |
|---|---:|---|
| 원본 행 | 968행 | 원본 CSV에 보존 |
| 완료 경기 행 | 864행 | 분석에 사용 |
| 미완료·취소 일정 | 104행 | `status != completed`이므로 분석에서 제외 |
| 분석용 필수값 누락 | 0건 | 추가 처리 불필요 |
| 팀-경기 ID 중복 | 0건 | 추가 처리 불필요 |
| 이상치 후보(절대 득실차 10점 이상) | 47경기, 팀 관점 49행 | 공식 경기센터 표본 대조 후 실제 결과로 유지 |
...
- 트렌드·계절 패턴 비교: 월별 승률은 달 단위 패턴을, 최근 10경기 승률은 짧은 기간의 추세를 보여 주도록 같은 2025년 경기 순서 위에 함께 표시했다. 두 선이 다를 때는 집계 단위에 따라 해석이 달라질 수 있음을 확인한다.
...
### 7.5 트렌드와 월별 패턴 비교 (반례 확인)

![2025년 월별 승률과 최근 10경기 승률 비교](images/07_monthly_vs_rolling_2025.png)

월별 승률은 한 달 전체를 묶어 보므로 달 단위 패턴을 보여 주고, 최근 10경기 승률은 짧은 기간의 변화를 더 민감하게 보여 준다. 예를 들어 KIA는 2025년 7월 월별 승률이 0.333이지만 7월 26일 시점 최근 10경기 승률은 0.100이었다. 반대로 삼성은 2025년 10월 월별 승률이 0.000(1경기 1패)이지만, 같은 날 최근 10경기 승률은 0.700이었다. 즉 10월 0%만 보고 삼성의 단기 전력이 매우 낮았다고 해석하면 반례가 된다.

이 비교는 엄밀한 장기 시계열 분해가 아니라, **월별 집계와 이동 승률이라는 서로 다른 시간 단위의 비교**다. 경기 수가 적은 월이나 10경기 창의 시작·끝 구간에서는 결과가 민감하게 달라질 수 있으므로 두 지표를 함께 해석했다.
...
- Action: 다음 분석에서는 두 팀의 투수·수비·득실차 데이터를 추가해 2024→2025 변화가 어느 지표에서 나타났는지 우선 확인한다.
...
- Action: 월별 그래프와 최근 10경기 그래프를 함께 보고, 경기 수가 적은 3·10월은 별도 표기해 성급한 해석을 피한다.
...
- Action: 다음 분석에서는 타자 한 명이 아닌 팀 타격 지표와 주요 투수 기록을 함께 추가해 개인 기록의 대표성 한계를 줄인다.
...
추가로 수집할 데이터의 우선순위는 아래와 같다.

| 우선순위 | 데이터 | 기대 효과 | 획득 난이도 |
|---:|---|---|---|
| 1 | 팀 득실차·팀 타격·팀 투구 지표 | 승률 변화의 내부 지표를 비교 | 낮음: KBO 기록에서 확인 가능 |
| 2 | 선수별 출전 경기·부상 공백 | 월별 성적 변화의 맥락 확인 | 중간: 공식 기록·기사 대조 필요 |
| 3 | 상대팀·원정·구장별 성적 | 일정과 상대 전력 효과 확인 | 중간: 현재 경기 데이터에서 추가 집계 가능 |
...
검증에 사용한 표본 게임 ID는 `20240731OBHT0`, `20250902HTHH0`, `20240328SSLG0`, `20240720LTSS0`, `20230928HTNC0`, `20230928SSLG0`이다. `python analysis.py`를 실행하면 시즌 합계가 KBO 공식 역대 구단성적과 일치하는지 자동 검증한다.

| AI 도움을 받은 산출 | 작성자의 확인·수정 |
|---|---|
| 수집·전처리·집계·그래프·예측 코드 초안 | 코드 실행 후 행 수, 중복, 공식 시즌 기록, 그래프 표시를 직접 확인하고 수정 |
| 보고서 문장 초안 | 수치가 CSV·그래프와 일치하는지 확인하고, 원인 단정 문장을 가능한 해석으로 완화 |
| 예측 방법 후보 | 2023~2024년으로 2025년을 먼저 검증한 뒤, 오차와 한계를 보고서에 추가 |
